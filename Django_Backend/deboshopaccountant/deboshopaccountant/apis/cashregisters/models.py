...
class CashRegisterModel(models.Model):
    cash_register_id = models.AutoField('ID', primary_key=True)
...
class CashRegisterOpeningModel(models.Model):
    cash_register_opening_id = models.AutoField('ID', primary_key=True)
    fk_cash_register = models.ForeignKey(
        'cashregisters.CashRegisterModel', on_delete=models.CASCADE, verbose_name='Caja')
    cash_register_opening_date = models.DateTimeField('Fecha')
    cash_register_opening_amount = models.DecimalField(
        'Monto de Apertura', max_digits=11, decimal_places=2)
...
class CashRegisterMovementsModel(models.Model):
    cash_register_movements_id = models.AutoField('ID', primary_key=True)
    fk_cash_register = models.ForeignKey(
        'cashregisters.CashRegisterModel', on_delete=models.CASCADE, verbose_name='Caja')
    cash_register_movements_date = models.DateTimeField('Fecha')
    cash_register_movements_detail = models.CharField(
        'Detalle', max_length=500)
...
class CashRegisterClosingModel(models.Model):
    cash_register_closing_id = models.AutoField('ID', primary_key=True)
    fk_cash_register = models.ForeignKey(
        'cashregisters.CashRegisterModel', on_delete=models.CASCADE, verbose_name='Caja')
    ...
    cash_register_closing_total_voucher_transactions = models.DecimalField(
        'Total de Ventas', max_digits=11, decimal_places=2)
    cash_register_closing_total_cash_equivalent = models.DecimalField(
        'Total Transferencias', max_digits=11, decimal_places=2)
    cash_register_closing_total_cash = models.DecimalField(
        'Total en Efectivo', max_digits=11, decimal_places=2)
    cash_register_closing_missing_or_surplus = models.DecimalField(
        'Faltante o Excedente', max_digits=11, decimal_places=2)
    cash_register_closing_amount = models.DecimalField(
        'Monto de Cierre', max_digits=11, decimal_places=2)
    cash_register_closing_remark = models.CharField(
        'Observación', max_length=1024, default='Ninguna')
