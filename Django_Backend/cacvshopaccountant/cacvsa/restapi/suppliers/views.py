...
class SupplierViewSet(GenericViewSet):
    ...
    def get_queryset(self):
        if self.queryset is None:
            return self.model.objects.filter(supplier_status=True)
    ...
    def destroy(self, request, pk=None):
        queryres = self.model.objects.filter(
            supplier_id=pk).update(supplier_status=False)
        if queryres == 1:
            data = {'msg': 'OK'}
            return Response(data, HTTP_200_OK)
        data = {'msg': 'ERROR'}
...
class IncomeViewSet(GenericViewSet):
    ...
    def get_queryset(self):
        if self.queryset is None:
            return self.model.objects.filter(income_status=True)
        return self.queryset
    ...
    def destroy(self, request, pk=None):
        queryres = self.model.objects.filter(
            income_id=pk).update(income_status=False)
        if queryres == 1:
            data = {'msg': 'OK'}
            return Response(data, HTTP_200_OK)
        data = {'msg': 'ERROR'}
...
class IncomeDetailViewSet(GenericViewSet):
